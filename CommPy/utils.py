...
__all__ = ['dec2bitarray', 'decimal2bitarray', 'bitarray2dec', 'hamming_dist', 'euclid_dist', 'signal_power', 
        'hermitian', 'bit_err_rate', 'symbol_err_rate', 'Qbase2dec', 'decimal2Qbase']
...
def symbol_err_rate(x_indices, y_indices):
    length = x_indices.shape[0]
    err_vec = np.zeros([length, ])
    for k in range(0, length):
        if x_indices[k] != y_indices[k]:
            err_vec[k] = 1
